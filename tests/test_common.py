...
def test_top_row():
    ...
    assert cm.top_row(test_board, 2) == 2
    assert cm.top_row(test_board, 3) == 3
    assert cm.top_row(test_board, 4) == 4
    assert cm.top_row(test_board, 5) == 0
    assert cm.top_row(test_board, 6) == 0
    try:
        assert not cm.top_row(test_board, 1)
    except IndexError:
        assert True
